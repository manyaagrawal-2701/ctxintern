...
class ReportGenerator:
    ...
    def create_visualizations(self, df: pd.DataFrame, user_id: int | None = None) -> dict:
        ...
        try:
            ...
            # 6. Recommended Transmission Type Share Chart (Replaces confusing correlation heatmap)
            trans_col = "transmission"
            if trans_col in df.columns:
                plt.figure(figsize=(5, 4))
                df[trans_col].value_counts().plot(kind="pie", autopct="%1.1f%%", colors=["#67e8f9", "#cbd5e1"])
                plt.ylabel("")
                plt.title("Transmission Distribution", fontsize=12, weight="bold")
                plt.tight_layout()
                path = img_dir / f"transmission_preference{suffix}.png"
                plt.savefig(path, dpi=120)
                plt.close()
                charts["transmission_preference"] = path
