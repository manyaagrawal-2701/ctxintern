...
#          performs hyperparameter tuning via GridSearchCV, evaluates their
#          accuracy/F1 metrics, and saves the winner.
# ==============================================================================
...
class XGBoostWrapper(BaseEstimator, ClassifierMixin):
    """Wrapper around XGBClassifier to handle string target labels seamlessly in Scikit-Learn."""

    def __init__(self, n_estimators=100, max_depth=6, learning_rate=0.1, eval_metric="mlogloss", random_state=42):
...
class ModelTrainer:
    """
    Manages model initialization, training, hyperparameter tuning, evaluation, and serialization.
    """
    ...
    # --------------------------------------------------------------------------
    # EVALUATION & COMPARISON WITH HYPERPARAMETER TUNING
    # --------------------------------------------------------------------------
    ...
    def compare_models(self, X_train, X_test, y_train, y_test):
        """
        Trains, tunes and compares all models in our dictionary.
        Uses GridSearchCV to find optimal hyperparameters for each classifier.
        ...
        # Parameter Grids for each classifier to optimize performance
        param_grids = {
            "Random Forest": {
        ...
        for name, model in self.models.items():
            logging.info(f"Tuning and training {name}...")
            print(f"Tuning hyperparameters for {name} using GridSearchCV...")
            
            grid = param_grids.get(name, {})
            if grid:
                # 3-Fold cross-validation to find best hyperparameters quickly and robustly
            ...
            # 2. Evaluate scores of the optimized model
            metrics = self.evaluate_model(best_model, X_test, y_test)
            metrics["Model"] = name
            self.results.append(metrics)

            # 3. Track the overall winner based on F1 Score
